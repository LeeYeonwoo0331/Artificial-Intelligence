...
# -----------------------------------------------------------------------------
your_id:
    .word   0x0141FF28                  # Write your student id
your_password:
    .word   0x0141FF28                  # Write your phase1 password
    .word   0xAF8035C0                  # Write your phase2 password
...
Loop1:
    bge t2, t1, phase4_unlock_code_exit
    slli t4, t2, 2
    add s1, t0, t4
    lw t4, 0(s1)
    addi t3, t2, -1
...
phase4_check1:
    ...
    add t4, t0, t4
    lw  t5, 0(t4)
    blt t3, t5, phase4_something_wrong
    addi t1, t1, 1
    jal x0, phase4_check1

phase4_check2:
    la a0, phase4_data
    jal ra, phase4_device
    add s3, a0, x0
    bne s2, s3, phase4_something_wrong
    addi a0, x0, 0
    jal x0, phase4_exit
...
phase4_device_loop:
    bge t1, t5, phase4_device_loop_exit
    slli t2, t1, 2
    add t3, t6, t2
    lw  t4, 0(t3)
    xor t0, t0, t4
    addi t1, t1, 1
    jal x0, phase4_device_loop
